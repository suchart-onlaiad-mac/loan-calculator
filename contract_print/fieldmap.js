...
window.FIELD_MAP = {
  ...
  /* ---- แถว "รวมยอด" ใต้ตาราง หน้า 2 ----
   * ฟอร์มมีช่องนี้มาตั้งแต่ต้น แต่ engine ไม่เคยเติม → ทุกใบที่พิมพ์ออกไปมีช่องว่าง (พบ 08-08-2569)
   * y = 306.4 มาจาก pdftotext -bbox ของคำว่า "รวมยอด" เอง (ไม่ใช่ค่าที่กะ)
   *     สอบทานกับการต่อแถว: 118.0 + 10*18.853 = 306.5 → ตรงกัน 2 วิธี
   * x = cols.principal เดิม → ตัวเลขรวมยอดอยู่ตรงแนวเดียวกับต้นเงินที่อยู่เหนือมันพอดี
   *     (กล่องในฟอร์มกินช่วง ~235–356pt = คอลัมน์ "ชำระต้นเงิน" ช่องเดียว ยืนยันด้วยภาพ) */
  tableTotal:  { page:2, x:275, y:306.4, size:10 },
