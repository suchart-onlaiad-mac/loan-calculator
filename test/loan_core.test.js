...
module.exports = function (T) {
  ...
  /* ค่าหุ้นตามข้อบังคับ ข้อ 6(2) — "ห้าหุ้นต่อจำนวนเงินกู้ทุก ๆ หนึ่งพันบาท
   * เศษของหนึ่งพันบาทให้ถือเป็นห้าหุ้น" · หุ้นละ 10 บาท (ข้อ 5)
   * 🔑 เกณฑ์คำนวณมาจากถ้อยคำระเบียบ ไม่ใช่จากสูตรในโค้ด
   * มติ ผจก 21-07-2569: ใช้ข้อบังคับ ยกเลิกกติกา "5% ตายตัว" ของ 20-07 */
  T.test('ค่าหุ้น: 5 หุ้นต่อทุกพันบาท เศษของพันปัดขึ้น (ข้อบังคับ ข้อ 6(2))', () => {
    const b = P => C.shareRequired(P).baht;
    const u = P => C.shareRequired(P).units;

    T.eq(u(10000), 50, '10,000 = 10 พัน → 50 หุ้น');
    T.eq(b(10000), 500, '50 หุ้น × 10 บาท = 500');
    T.eq(b(1000), 50, 'พันเดียว → 5 หุ้น = 50 บาท');
    T.eq(b(100000), 5000, 'ลงตัวพัน → เท่ากับ 5% พอดี');
    T.eq(b(250000), 12500, 'ลงตัวพัน → เท่ากับ 5% พอดี');

    // จุดที่ต่างจากกติกาเดิม 5% — เศษของพันต้องปัดขึ้นเต็ม 5 หุ้น
    T.eq(b(10001), 550, 'เศษ 1 บาท → ปัดขึ้นเป็น 11 พัน = 55 หุ้น = 550 บาท');
    T.eq(b(10500), 550, 'เศษ 500 → ยังเป็น 11 พัน');
    T.eq(b(10999), 550, 'เศษ 999 → ยังเป็น 11 พัน');
    T.eq(b(11000), 550, 'ลงตัว 11 พันพอดี = ค่าเดียวกัน');
    T.eq(b(123456), 6200, 'เศษ 456 → 124 พัน = 620 หุ้น = 6,200 บาท');

    // ต้องไม่เก็บน้อยกว่าเดิมทุกกรณี (เดิม = 5% ปัดปกติ)
    for (let P = 1; P <= 200000; P += 137) {
      T.ok(b(P) >= Math.round(P * 0.05), 'ต้องไม่ต่ำกว่ากติกาเดิม 5% ที่ P=' + P);
      T.ok(b(P) - Math.round(P * 0.05) <= 50, 'ต่างจากเดิมไม่เกิน 50 บาท ที่ P=' + P);
    }

    T.eq(b(0), 0, 'วงเงิน 0 → ไม่มีค่าหุ้น');
    T.eq(b(-100), 0, 'วงเงินติดลบ → 0 ไม่ใช่ค่าติดลบ');
    T.eq(b(NaN), 0, 'NaN → 0');
    T.eq(b(1), 50, '1 บาท → ยังต้องถือ 5 หุ้น (เศษของพัน)');
  });

  /* เพดานรวมทุกสัญญา — ระเบียบ 101 ข้อ 6 วรรคท้าย
   * "เงินกู้ระยะสั้นและระยะปานกลางรวมทุกรายการ ... จะเกิน 1,000,000 บาท ไม่ได้"
   * เกณฑ์มาจากข้อความระเบียบ ไม่ใช่จากสูตรในโค้ด */
  T.test('เพดานรวม: นับเฉพาะระยะสั้น+ปานกลาง · ระยะยาวไม่นับ (ข้อ 6 วรรคท้าย)', () => {
    T.eq(C.TOTAL_CEILING, 1000000, 'เพดานรวมตามระเบียบ');
    T.eq(C.loanTotalCounted(500000, []).sum, 500000, 'ไม่มีหนี้เดิม = วงเงินใหม่');
    T.eq(C.loanTotalCounted(600000, [{ type: 'ระยะสั้น', remain: '500000' }]).sum, 1100000,
      'ใหม่ 600,000 + เดิม 500,000 = 1.1 ล้าน (เกิน)');
    T.eq(C.loanTotalCounted(600000, [{ type: 'ระยะยาว', remain: '500000' }]).sum, 600000,
      'ระยะยาวไม่นับเข้าเพดานข้อนี้');
    T.eq(C.loanTotalCounted(600000, [{ type: 'ระยะยาว', remain: '500000' }]).skipped.length, 1,
      'ระยะยาวถูกบันทึกว่าข้าม เพื่อบอกเจ้าหน้าที่ได้');
    T.eq(C.loanTotalCounted(100000, [
      { type: 'ระยะสั้น', remain: '200,000' },
      { type: 'ระยะปานกลาง', remain: '300000' },
      { type: 'ระยะยาว', remain: '900000' },
    ]).sum, 600000, 'มีลูกน้ำในตัวเลข + คละประเภท');
    T.eq(C.loanTotalCounted(100000, [{ type: '', remain: '500000' }]).sum, 100000,
      'แถวที่ยังไม่เลือกประเภท = ไม่นับ (ยังกรอกไม่เสร็จ)');
    T.eq(C.loanTotalCounted(100000, [{ type: 'ระยะสั้น', remain: '' }]).sum, 100000,
      'เลือกประเภทแต่ยังไม่ใส่ยอด = ไม่นับ');
    T.eq(C.loanTotalCounted(100000, [{ type: 'ระยะสั้น', remain: 'abc' }]).sum, 100000,
      'ยอดที่อ่านเป็นตัวเลขไม่ได้ = ไม่นับ');
    T.eq(C.loanTotalCounted(1000000, []).sum, 1000000, 'ขอบพอดี 1 ล้าน = ยังไม่เกิน');
    T.eq(C.loanTotalCounted(1000001, []).sum, 1000001, 'เกิน 1 บาท');
    T.eq(C.loanTotalCounted(0, null).sum, 0, 'ไม่ส่งตารางหนี้มาเลย → ไม่พัง');
  });
